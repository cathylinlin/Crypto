...
造格就是造一个大方阵，这个大方阵乘上某个行向量后就能得到一个范数很小的行向量，而且行向量包含有用的信息。   

### 2 一个简单的ACD问题   
已知 $l_i=a_ip+b_i$，i到30，`a`大约`p^2`那么大，`b`小于`p`，如何求`p`？  

我想要什么？   
那就来个`a`吧。  

`l1`很大，那`a1l2-a2l1`呢？
$a_1l_2-a_2l_1=a_1b_2-a_2b_1$，后者有多大呢？   

论文告诉我们，如此相乘相减，的确可以得到一个比较小的数。  
...
和谁相乘？ 和 $(a_1,a_2,...,a_i)$相乘，得到 $(?a_1,a_1l_2-a_2l_1,a_1l_3-a_3l_1,...,a_1l_i-a_il_1)$   
问号是什么？问号就是你怎么配平使之满足hermite定理。   

然后得到`a1`，最后`l1//a1`就得到`p`了。   
